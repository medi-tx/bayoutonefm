...
(function(){
  ...
  function sotdAvatarHtml(p){
    if(p && p.photo) return `<span class="sotd-avatar"><img loading="lazy" decoding="async" src="${escapeAttr(p.photo)}" alt="Profile photo" style="width:100%;height:100%;object-fit:cover;"></span>`;
   ...
   function sotdRenderHistory(){
     ...
     listEl.innerHTML = filtered.map(item => {
       ...
       const cover = song.cover
         ? `<img loading="lazy" decoding="async" class="sotd-history-cover" src="${escapeAttr(song.cover)}" alt="Album cover">`
       ...
       if(item.reactions.length > 0){
         reactionsHtml = item.reactions.map(r => {
           ...
           const avatar = (p && p.photo)
             ? `<span class="sotd-avatar"><img loading="lazy" decoding="async" src="${escapeAttr(p.photo)}" alt="Profile photo" style="width:100%;height:100%;object-fit:cover;"></span>`
