...
async function syncCovers(songList, uid){
  if(!uid || !sb || !songList || !songList.length) return;
  try{
    const rows = [];
    ...
    for(const s of songList){
      ...
      if(s.coverArt){
        withCover.add(s.id);
        rows.push({ user_id: uid, song_id: s.id, cover_art: String(s.coverArt), updated_at: new Date().toISOString() });
      }
    }
    ...
    for(let i = 0; i < noCover.length; i += 50){
      ...
      const { error } = await sb.from('song_covers').delete().eq('user_id', uid).in('song_id', ids);
      if(error) throw error;
    }
...
async function doSync(attempt, startRevision){
  ...
  try{
    ...
    await syncCovers(slimSongs, uid);
    let payload = { user_id: uid, songs: slimSongs, people, wishlist, updated_at: updated };
    if(JSON.stringify(payload).length < 900000){
      ...
      await pushUserData({ user_id: uid, people, wishlist, updated_at: updated });
      const { data: existing } = await sb.from('user_data').select('songs').eq('user_id', uid).maybeSingle();
      const remoteOnly = (((existing && existing.songs) || [])).filter(s => !localIds.has(s.id));
      const merged = [...slimSongs, ...remoteOnly];
      payload = { user_id: uid, songs: merged, updated_at: updated };
      if(JSON.stringify(payload).length < 900000){
        await pushUserData(payload);
      } else {
