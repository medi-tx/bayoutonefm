...
function feedImportCardHtml(clusterName, entries, who, profile, ownerId, when){
  ...
  const avatar = (profile && profile.photo)
    ? `<span class="feed-card-avatar"><img src="${escapeAttr(profile.photo)}" alt=""></span>`
  ...
  const previewCovers = entries.slice(0,3).map(e=> e.song.coverArt ? `<img src="${escapeAttr(e.song.coverArt)}" style="width:32px;height:32px;border-radius:4px;object-fit:cover;">` : '').join('');
  const songList = entries.map(e=>`<div style="display:flex;align-items:center;gap:8px;padding:4px 0;font-size:13px;border-bottom:1px solid rgba(var(--on-paper-rgb),0.08);">
    ${e.song.coverArt ? `<img src="${escapeAttr(e.song.coverArt)}" style="width:28px;height:28px;border-radius:3px;object-fit:cover;flex-shrink:0;">` : ''}
