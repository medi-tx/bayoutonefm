...
function openImportUrlScreen(mode){
  ...
  if(addMusicMode === 'song'){ titleEl.textContent = 'Add a song'; labelEl.textContent = 'Paste a Spotify, Apple Music, or Tidal song URL'; }
  else if(addMusicMode === 'album'){ titleEl.textContent = 'Add an album'; labelEl.textContent = 'Paste a Spotify, Apple Music, or Tidal album URL'; }
  else { titleEl.textContent = 'Import Playlist'; labelEl.textContent = 'Playlist URL (Spotify, Apple Music, or Tidal)'; }
...
async function handleSingleUrl(url){
  ...
  try{
    ...
    if(isSpotifyAlbum){
      ...
      const albumData = { artists:(a.artists||[]).map(x=>x.name), album:a.name||'', year:a.release_date||'', coverArt:(a.images&&a.images[0]&&a.images[0].url)||null, tracks:(a.tracks&&a.tracks.items||[]).map(t=>({ title:t.name, artists:(t.artists||[]).map(x=>x.name), trackNumber:t.track_number })) };
      document.getElementById('spotifyImportOverlay').classList.remove('open');
      openMultiModal('album');
      document.getElementById('mf-artist').value = albumData.artists.join(', ');
      document.getElementById('mf-album').value = albumData.album;
      document.getElementById('mf-year').value = albumData.year;
      if(albumData.coverArt){ currentMultiCoverArt = albumData.coverArt; setImagePreview('mf-cover', currentMultiCoverArt); }
      const boxes = document.getElementById('titleBoxes');
      boxes.innerHTML = '';
      albumData.tracks.forEach((t,i)=> addTitleBoxRow(i===0, t.title));
      return;
    }
    ...
    if(isTidalTrack){
      ...
      for(const proxy of proxies){
        try{
          ...
          const resp = await fetch(proxy('https://tidal.com/browse/track/' + trackId), { redirect:'follow', signal:ac.signal });
          clearTimeout(timer);
          if(resp.ok){
            ...
            if(ldScript){
              ...
              if(ld){
                const trackData = { title:ld.name||'', artists:ld.artist?[ld.artist]:[], album:ld.album&&ld.album.name||'', year:ld.dateCreated||'', coverArt:ld.image||null, tidalUrl:url };
                document.getElementById('spotifyImportOverlay').classList.remove('open');
                openAddFromData(trackData);
    ...
    if(isTidalAlbum){
      ...
      for(const proxy of proxies){
        try{
          const ac = new AbortController();
          const timer = setTimeout(()=>ac.abort(), 6000);
          const resp = await fetch(proxy('https://tidal.com/browse/album/' + albumId), { redirect:'follow', signal:ac.signal });
          clearTimeout(timer);
          if(resp.ok){
            const html = await resp.text();
            const doc = new DOMParser().parseFromString(html, 'text/html');
...
document.getElementById('spotify-url-input').addEventListener('input', e=>{
  ...
  if(addMusicMode === 'song' || addMusicMode === 'album'){
