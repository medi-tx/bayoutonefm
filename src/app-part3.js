...
async function drawSongShareCard(opts){
  ...
  if(window.html2canvas && frontEl){
    try{
      ...
      const c = await html2canvas(frontEl, { backgroundColor: loadTheme().paper || '#eedd95', scale: 2 });
...
async function drawSongBackShareCard(opts, cvId){
  ...
  if(window.html2canvas && backEl){
    try{
      ...
      await new Promise(r=>setTimeout(r,150));
      const c = await html2canvas(backEl, { backgroundColor: loadTheme().paper || '#eedd95', scale: 2 });
      cv.width = c.width; cv.height = c.height;
      const ctx = cv.getContext('2d');
      ctx.drawImage(c, 0, 0);
      return;
