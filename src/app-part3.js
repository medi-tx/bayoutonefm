...
document.getElementById('becomeTesterBtn').addEventListener('click', ()=>{
...
const SHARE_SHEET_W = 1080;
const SHARE_SHEET_H = 1350;
const SHARE_SHEET_PAD = 64;
function fitCardIntoSheet(src){
  const W = SHARE_SHEET_W, H = SHARE_SHEET_H, pad = SHARE_SHEET_PAD;
...
function applySheetToTarget(targetId, src){
  const cv = document.getElementById(targetId);
  if(!cv) return;
  const norm = fitCardIntoSheet(src);
  cv.width = norm.width; cv.height = norm.height;
  cv.getContext('2d').drawImage(norm, 0, 0);
}
async function drawSongShareCard(opts){
  ...
  ctx.fillStyle='#eedd95'; ctx.fillText('Join bayoutonefm today to share your own music', W/2, H - 96);
  ctx.textAlign = 'left';
  const frontSnap = cv.toDataURL('image/png');
  const backCv = document.getElementById('shareCanvasBack');
  if(backCv) await drawSongBackShareCard(opts, 'shareCanvasBack');
  const normImg = new Image();
  normImg.onload = function(){ applySheetToTarget('shareCanvas', normImg); };
  normImg.src = frontSnap;
}
...
function addTitleBoxRow(focus, prefill){
  ...
  if(isAlbum){
    ...
    if(!trackTier && prefill && prefill.score){
      trackTier = tierForScore(Math.max(30, Math.min(100, prefill.score)));
    }
    ['★','S','A','B','C'].forEach(t=>{
      const btn = document.createElement('button');
      btn.type = 'button';
    ...
    scoreInput.addEventListener('input', ()=>{
      ...
      if(isNaN(sv)) return;
      const autoTier = tierForScore(Math.max(30, Math.min(100, sv)));
      if(autoTier){
        trackTier = autoTier;
        tierRow.querySelectorAll('.track-tier-btn').forEach(b=>b.classList.toggle('selected', b.textContent===trackTier));
      }
...
function openImportUrlScreen(mode){
  addMusicMode = mode || 'song';
  const ov = document.getElementById('spotifyImportOverlay');
  ov.classList.add('open');
  const titleEl = document.getElementById('importModalTitle');
  const labelEl = document.getElementById('spotifyUrlLabel');
  const isAlbum = addMusicMode === 'album';
  titleEl.textContent = isAlbum ? 'Add an Album' : 'Add Music';
  labelEl.textContent = isAlbum ? 'Or paste an Apple Music, Spotify, or Tidal album link' : 'Paste an Apple Music, Spotify, YouTube, or Tidal link';
  ...
  playlistImportService = 'apple';
  playlistImportUrl = null;
  playlistImportLoadedUrl = null;
  playlistImportInProgress = false;
  stopImportProgressTimer();
  if(isAlbum){ document.getElementById('albumSearchInput').focus(); }
  else { document.getElementById('spotify-url-input').focus(); }
}
...
/* ---- PLAYLIST IMPORT (Apple Music / YouTube / Tidal) ---- */
let playlistImportedTracks = [];
let playlistImportService = 'apple';
...
async function finishListen(blob){
  ...
  // Overall watchdog: whatever happens, release the hourglass so the UI can't lock.
  let watchdog = setTimeout(()=>{
    const b = document.getElementById('listenMicBtn');
    if(b){ b.disabled = false; b.textContent = '🎤'; }
    const st = document.getElementById('listenStatus');
    if(st && st.textContent.indexOf('catch it') === -1 && st.textContent.indexOf('failed') === -1){
      st.textContent = 'That took too long — close this and give it another try.';
    }
    console.error('Live Listen watchdog fired: identification exceeded 60s');
  }, 60000);
  const release = (msg)=>{
    clearTimeout(watchdog);
    micBtn.disabled = false;
    micBtn.textContent = '🎤';
    if(msg) listenStatusText(msg);
  };
  let sig = null;
  try{
    const mod = await loadShazamCore();
    listenStatusText('Building fingerprint…');
    const pcm = await withTimeout(blobToPcm16k(blob), LISTEN_FETCH_TIMEOUT, 'decode_audio_timeout');
    const wav = pcmToWav(pcm, 16000);
    const sigs = mod.recognizeBytes(wav);
    if(sigs && sigs.length){ sig = sigs[0]; }
  }catch(err){
    console.error('Fingerprint error:', err);
    release("Couldn't build a fingerprint — check your connection and try again.");
    return;
  }
  if(!sig || !sig.uri){
    release('Couldn\'t catch it. Move closer to the speaker and try again.');
    return;
  }
  const payload = {
  ...
  for(let attempt = 1; attempt <= LISTEN_ATTEMPTS; attempt++){
    ...
    try{
      ...
      release('Identification failed — check your connection and try again.');
      return;
    }
  }
  release('Couldn\'t catch it. Move closer to the speaker and tap the mic to try again.');
}
...
async function buildAlbumFromItunesId(albumId){
  ...
  try{
    ...
    errEl.innerHTML = '<p class="profile-empty-note">' + escapeHtml(e.message||'Could not load this album.') + '</p>';
    return false;
  }
}
...
async function buildAlbumFromDeezerId(albumId){
  ...
  try{
    ...
    const tracks = (albumInfo.tracks.data||[]).map((t,i)=>({
      ...
      coverArt:albumInfo.cover_big ? albumInfo.cover_big.replace('/500x500','/150x150') : null, appleMusicUrl:''
    }));
    const albumName = albumInfo.title || '';
    const albumArtist = albumInfo.artist ? albumInfo.artist.name : '';
    const albumYear = albumInfo.release_date || '';
    const coverArt = albumInfo.cover_big || null;
    albumBuildMeta = { albumName, albumArtist, albumYear, coverArt, clusterId: uid() };
    albumBuildTracks = tracks.map((t,i)=>({
      title:t.title, artists:(t.artists&&t.artists.length)?t.artists:[albumArtist], trackNumber:t.trackNumber||(i+1), coverArt:t.coverArt||coverArt, appleMusicUrl:t.appleMusicUrl||'',
      album:albumName, year:albumYear||null, releaseDate:albumYear||null, tier:null, score:null,
    ...
    console.error('Deezer album lookup error:', e);
    errEl.style.display = 'block';
    errEl.innerHTML = '<p class="profile-empty-note">' + escapeHtml(e.message||'Could not load this album.') + '</p>';
    return false;
  }
...
async function searchAlbums(term){
  ...
  resultsEl.innerHTML = '<p class="msg-empty" style="opacity:0.7;">Searching…</p>';
  const apples = itunesSearch(term, 'album', 8);
  const deezers = deezerAlbumSearch(term, 8);
  try{
    const [appleResults, deezerResults] = await Promise.all([apples, deezers]);
    const seen = new Set();
    const combined = [];
    deezerResults.forEach(r=>{
      const key = r.collectionName.toLowerCase() + '|' + (r.artistName||'').toLowerCase();
      if(seen.has(key)) return;
      seen.add(key);
      combined.push({ id: r.collectionId, name: r.collectionName, artist: r.artistName, year: (r.releaseDate||'').substring(0,4), coverArt: r.artworkUrl100, provider: 'deezer' });
    });
    appleResults.forEach(r=>{
      const key = (r.collectionName||'').toLowerCase() + '|' + (r.artistName||'').toLowerCase();
      if(seen.has(key)) return;
      seen.add(key);
      combined.push({ id: r.collectionId, name: r.collectionName, artist: r.artistName, year: (r.releaseDate||'').substring(0,4), coverArt: r.artworkUrl100, provider: 'apple' });
    });
    if(!combined.length){
      resultsEl.innerHTML = '<p class="msg-empty">No albums found. Try a different search or paste a link.</p>';
      return;
    }
    resultsEl.innerHTML = combined.map(a=>`
      <div class="discover-row" style="cursor:pointer; align-items:center;" data-ab-search="${escapeHtml(a.id)}">
        ${a.coverArt
          ? `<img loading="lazy" decoding="async" src="${escapeAttr(a.coverArt)}" style="width:36px;height:36px;border-radius:5px;object-fit:cover;flex-shrink:0;" alt="">`
          : `<span class="drow-fallback" style="font-size:11px;">${escapeHtml((a.name||'?').charAt(0).toUpperCase())}</span>`}
        <span style="flex:1; min-width:0;">
          <span class="drow-name">${escapeHtml(a.name)}</span><br>
          <span class="drow-bio">${escapeHtml(a.artist + (a.year ? ' · ' + a.year : ''))}</span>
        </span>
      </div>`).join('');
    resultsEl.querySelectorAll('[data-ab-search]').forEach(row=>{
      row.addEventListener('click', async ()=>{
        ...
        if(ok){
          document.getElementById('albumSearchField').style.display = 'none';
          document.getElementById('spotifyUrlLabel').style.display = 'none';
    ...
    resultsEl.innerHTML = '<p class="msg-empty">Could not search for albums. Try pasting a link instead.</p>';
  }
}
...
function importAlbumBuildTracks(){
  ...
  document.getElementById('spotifyUrlLabel').style.display = '';
  document.getElementById('albumSearchField').style.display = 'none';
  render();
...
function cancelAlbumBuild(){
  ...
  document.getElementById('spotify-url-input').style.display = '';
  document.getElementById('spotifyUrlLabel').style.display = '';
  document.getElementById('albumSearchField').style.display = 'block';
}
...
function detectImportType(url){
  if(/music\.apple\.com\/.*\/playlist\//.test(url)) return { service:'apple', kind:'playlist' };
  if(/music\.apple\.com\/.*\/album\/.*\/(\d+)(?:\?i=\d+)?/.test(url)) return { service:'apple', kind:'album' };
  if(/tidal\.com\/(browse\/)?playlist\//.test(url)) return { service:'tidal', kind:'playlist' };
  if(/tidal\.com\/(browse\/)?album\/(\d+)/.test(url)) return { service:'tidal', kind:'album' };
  if(/music\.youtube\.com\/playlist|youtube\.com\/playlist/.test(url)) return { service:'youtube', kind:'playlist' };
  if(/open\.spotify\.com\/playlist\//.test(url)) return { service:'spotify', kind:'playlist' };
  if(/open\.spotify\.com\/album\/([a-zA-Z0-9]+)/.test(url)) return { service:'spotify', kind:'album' };
  return null;
}

async function loadAppleAlbum(url){
  const m = url.match(/music\.apple\.com\/.*\/album\/.*\/(\d+)(?:\?i=\d+)?/);
  const albumId = m[1];
  const resp = await fetch('https://itunes.apple.com/lookup?id=' + albumId + '&entity=song');
  if(!resp.ok) throw new Error('Apple Music could not load this album right now.');
  const d = await resp.json();
  ...
  if(!songs.length) throw new Error('Could not find any tracks in this Apple Music album. It may be unavailable.');
  const albumName = (col && col.collectionName) || (songs[0].collectionName || 'Apple Music Album');
  const coverArt = (col && col.artworkUrl100) || songs[0].artworkUrl100 || null;
  const tracks = songs
    .sort((a, b) => (a.trackNumber || 0) - (b.trackNumber || 0))
    .map(s => ({
      title: s.trackName,
      trackNumber: s.trackNumber || null,
      artists: [s.artistName].filter(Boolean),
      album: albumName,
      coverArt: coverArt,
      year: (s.releaseDate || '').substring(0, 4) || null,
      appleMusicUrl: s.trackViewUrl || null,
    }));
  return { name: albumName, tracks, coverArt };
}

async function loadSpotifyAlbum(url){
  ...
  const get = async path => {
    ...
    if(!resp.ok) throw new Error('Spotify could not load this album.');
    return resp.json();
  };
  const d = await get('albums/' + albumId);
  const albumName = d.name || '';
  const albumArtists = (d.artists || []).map(a => a.name).filter(Boolean);
  const coverArt = (d.images && d.images[0] && d.images[0].url) || null;
  const releaseDate = d.release_date || '';
  ...
  let offset = 0;
  while(offset < totalTracks || offset === 0){
    const td = await get('albums/' + albumId + '/tracks?limit=50&offset=' + offset);
    const items = td.items || [];
    if(!items.length) break;
    for(const t of items){
      tracks.push({
        ...
        artists: ((t.artists && t.artists.length ? t.artists.map(a => a.name) : albumArtists)).filter(Boolean),
        album: albumName,
        coverArt: coverArt,
  ...
  if(!tracks.length) throw new Error('Could not load tracks from this Spotify album.');
  return { name: albumName || 'Spotify Album', tracks, coverArt };
}

async function loadTidalAlbum(url){
  const albumId = url.match(/tidal\.com\/(?:browse\/)?album\/(\d+)/)[1];
  const token = await getTidalToken();
  const auth = { Authorization:'Bearer '+token, 'Content-Type':'application/vnd.tidal.v2+json' };
  const resp = await fetch('https://openapi.tidal.com/v2/albums?filter[id]='+albumId+'&include=artists&countryCode=US', { headers: auth });
  if(!resp.ok) throw new Error('Tidal could not load this album.');
  const d = await resp.json();
  const artistMap = {};
  (d.included || []).forEach(inc => {
    if(inc.type === 'artists' && inc.id) artistMap[inc.id] = inc.attributes && inc.attributes.name;
  });
  if(!d.data || !d.data[0]) throw new Error('Could not find this Tidal album.');
  const a = d.data[0];
  const attrs = a.attributes || {};
  const albumArtists = ((a.relationships && a.relationships.artists && a.relationships.artists.data) || [])
    .map(x => artistMap[x.id]).filter(Boolean);
  const coverArt = attrs.cover ? 'https://resources.tidal.com/images/'+attrs.cover.replace('/','-')+'/640x640.jpg' : null;
  const albumName = attrs.title || '';
  const trResp = await fetch('https://openapi.tidal.com/v2/albums/'+albumId+'/tracks?countryCode=US&limit=300', { headers: auth });
  let tracks = [];
  if(trResp.ok){
    const td = await trResp.json();
    const items = td.items || td.data || [];
    tracks = items.map((t, i) => {
      const trAttrs = t.attributes || {};
      return { title: trAttrs.title || '', trackNumber: trAttrs.trackNumber || (i + 1), artists: albumArtists, album: albumName, coverArt: coverArt, tidalUrl: url };
    });
  }
  if(!tracks.length || !tracks.some(t => t.title)) throw new Error('Could not load tracks from this Tidal album.');
  return { name: albumName || 'Tidal Album', tracks, coverArt };
}
...
async function loadPlaylist(url){
  ...
  const det = detectImportType(url);
  if(!det){
    errEl.style.display = 'block';
    errEl.innerHTML = '<p class="profile-empty-note">Please paste a supported Apple Music, Spotify, YouTube, or Tidal album or playlist URL.</p>';
    return;
  }
  ...
  try{
    ...
    if(det.kind === 'album'){
      if(service === 'apple'){
        const result = await loadAppleAlbum(url);
        playlistName = result.name; tracks = result.tracks; coverArt = result.coverArt;
      } else if(service === 'spotify'){
        const result = await loadSpotifyAlbum(url);
        playlistName = result.name; tracks = result.tracks; coverArt = result.coverArt;
      } else if(service === 'tidal'){
        const result = await loadTidalAlbum(url);
        playlistName = result.name; tracks = result.tracks; coverArt = result.coverArt;
      }
    ...
    if(!tracks || !tracks.length){
      listEl.innerHTML = '<p class="profile-empty-note">No tracks found in this album or playlist.</p>';
      document.getElementById('spotifyImportStopBtn').style.display = 'none';
      document.getElementById('spotifyImportPauseBtn').style.display = 'none';
      return;
    }
