...
function renderPeople(){
  ...
  row.innerHTML = people.map(p=>{
    ...
    <button type="button" class="person-card ${remindsFilterId===p.id?'active':''}${isExample?' example':''}${lp?' linked':''}" data-person="${p.id}" title="${tip}">
      ...
      ${photo ? `<img class="person-photo" src="${photo}">` : `<span class="person-photo-fallback">${escapeHtml((p.name||'?').charAt(0).toUpperCase())}</span>`}
...
function tierBoardItemHtml(s, editable){
  ...
  const cover = s.coverArt
    ? `<img class="tier-board-cover" src="${escapeAttr(s.coverArt)}" loading="lazy" decoding="async" alt="">
`
...
function coverThumbHtml(s){
  if(s && s.coverArt){
    return `<img class="cover-thumb" src="${escapeAttr(s.coverArt)}" loading="lazy" decoding="async" alt="">`;
  }
...
function songCardHtml(s, clusterCounts){
  ...
  return `
      <div class="card ${s.archived?'archived':''}${showEx?' example-card':''}" data-id="${s.id}">
        ...
        ${(s.tags&&s.tags.length) ? `<div class="tags">${s.tags.map(t=>`<span class="tag"${s.tier?` style="color:${tierColor(s.tier)};border-color:${tierColor(s.tier)}"`:''}>${escapeHtml(t)}</span>`).join('')}</div>` : (showEx ? `<div class="tags">${['nostalgic','road trip','late night'].map(t=>`<span class="tag ex-tag">e.g. ${t}</span>`).join('')}</div>` : '')}
        ...
        ${(s.remindsOf && s.remindsOf.length) ? `<div class="reminds-badges">${s.remindsOf.map(pid=>{
          ...
          return `<span class="reminds-badge${lp?' linked':''}" data-person="${p.id}" title="${tip}">${photo?`<img src="${photo}" loading="lazy" decoding="async">`:''}${escapeHtml(p.name)}${lp?' <span class="rc-linked">✓</span>':''}<button type="button" class="reminds-badge-x" data-remove-reminder="${s.id}|${p.id}" title="Remove ${escapeAttr(p.name)} from this song">×</button></span>`;
...
function timelineItemHtml(s){
  const cover = s.coverArt
    ? `<img class="timeline-cover" src="${escapeAttr(s.coverArt)}" loading="lazy" decoding="async" alt="">
`
  ...
  if(total === 0){
    ...
    if(topSongs.length){
      ...
      topSongs.forEach((s,i)=>{
        const cover = s.coverArt ? `<img class="yr-song-cover" src="${escapeAttr(s.coverArt)}" loading="lazy" alt="">` : '';
...
function mixtapeItemHtml(s, who){
  const cover = s.coverArt
    ? `<img class="mixtape-cover" src="${escapeAttr(s.coverArt)}" loading="lazy" decoding="async" alt="">
`
...
function discoverCardHtml(row, alreadyOwned){
  const cover = row.cover_art
    ? `<img class="mixtape-cover" src="${escapeAttr(row.cover_art)}" loading="lazy" decoding="async" alt="">`
...
/* ---- LISTEN: identify a song playing out loud (AudD.io) ---- */
const AUDD_TOKEN_KEY = 'bayoutonefm-audd-token';
const AUDD_PROXY_URL = SUPABASE_URL + '/functions/v1/audd-proxy';
let listenMediaStream = null;
let listenRecorder = null;
let listenChunks = [];
let listenTimer = null;
let listenUsingOwnToken = false; // whether this attempt is using the person's own AudD token

function listenToken(){
  try{ return localStorage.getItem(AUDD_TOKEN_KEY) || ''; }catch(e){ return ''; }
}
function openListenOverlay(){
  trackEvent('open_listen');
  const hasOwnToken = !!listenToken();
  // Everyone starts on the free shared listener — pasting a personal token is optional,
  // only needed if the shared daily quota runs out or someone wants unlimited use.
  document.getElementById('listenTokenSetup').style.display = 'none';
  document.getElementById('listenMain').style.display = '';
  document.getElementById('listenResultCard').style.display = 'none';
  document.getElementById('listenStatus').textContent = 'Tap the mic to start listening';
  document.getElementById('listenMicBtn').textContent = '🎤';
  document.getElementById('listenQuotaNote').textContent = hasOwnToken
    ? 'Using your own AudD token — unlimited listens.'
    : 'Free shared listener — a few identifies per day, no signup needed.';
  document.getElementById('listenOwnTokenLinkBtn').style.display = hasOwnToken ? 'none' : '';
...
document.getElementById('listenOwnTokenLinkBtn').addEventListener('click', ()=>{
  trackEvent('listen_show_own_token_form');
  document.getElementById('listenTokenSetup').style.display = '';
  document.getElementById('listenMain').style.display = 'none';
  document.getElementById('listenTokenInput').value = '';
});
document.getElementById('listenUseSharedBtn').addEventListener('click', ()=>{
  document.getElementById('listenTokenSetup').style.display = 'none';
  document.getElementById('listenMain').style.display = '';
});
document.getElementById('listenTokenSaveBtn').addEventListener('click', ()=>{
  const val = document.getElementById('listenTokenInput').value.trim();
  if(!val){ alert('Paste your AudD api_token first.'); return; }
  localStorage.setItem(AUDD_TOKEN_KEY, val);
  document.getElementById('listenTokenSetup').style.display = 'none';
  document.getElementById('listenMain').style.display = '';
  document.getElementById('listenQuotaNote').textContent = 'Using your own AudD token — unlimited listens.';
  document.getElementById('listenOwnTokenLinkBtn').style.display = 'none';
});
function stopListenRecording(silent){
  ...
  btn.textContent = '🎤';
  if(!silent){
    // status text is set by the caller
  }
}
async function identifyWithAudd(blob){
  // Own token: call AudD directly from the browser, as before.
  const form = new FormData();
  form.append('api_token', listenToken());
  ...
  let res;
  try{
    res = await fetch('https://api.audd.io/', { method:'POST', body: form, signal: ctrl.signal });
  } finally { clearTimeout(tmo); }
  if(!res.ok) throw new Error('audd_http_' + res.status);
  return await res.json();
}
async function identifyViaSharedProxy(blob){
  // No personal token: route through our own Supabase Edge Function, which holds a
  // shared AudD key server-side and rate-limits by account. See audd-proxy function.
  ...
  let res;
  try{
    res = await fetch(AUDD_PROXY_URL, {
      method: 'POST',
      headers: {
...
async function runListen(){
  ...
  if(!navigator.mediaDevices || !window.MediaRecorder){
    status.textContent = 'Your browser can\'t record audio — try Chrome or Safari.';
    return;
  }
  try{
  ...
  try{
    ...
    stopListenRecording(true);
    status.textContent = 'Couldn\'t start recording in this browser.';
    return;
  }
  listenRecorder.ondataavailable = e=>{ if(e.data && e.data.size) listenChunks.push(e.data); };
  listenRecorder.onerror = ()=>{ stopListenRecording(true); status.textContent = 'Recording error — try again.'; };
  micBtn.textContent = '⏹';
  let secs = 7;
  status.textContent = 'Listening… play the song! (' + secs + ')';
...
async function finishListen(blob){
  ...
  const hasOwnToken = !!listenToken();
  try{
    const data = hasOwnToken ? await identifyWithAudd(blob) : await identifyViaSharedProxy(blob);
    const r = data && data.result;
    if(!r || !r.title){
      status.textContent = 'Couldn\'t catch it. Move closer to the speaker and try again.';
      micBtn.textContent = '🎤';
      return;
    }
    const am = r.apple_music || {};
    document.getElementById('listenResultTitle').textContent = r.title;
    document.getElementById('listenResultArtist').textContent = r.artist || '';
    document.getElementById('listenResultAlbum').textContent = r.album || '';
    const art = am.artworkUrl100 || '';
    const artEl = document.getElementById('listenResultArt');
    artEl.src = art ? art.replace(/\/\d+x\d+/, '/300x300') : '';
    artEl.style.display = art ? '' : 'none';
    window._listenHit = {
      title: r.title,
      artists: [r.artist].filter(Boolean),
      album: r.album || '',
      year: (r.release_date || '').slice(0,4) || '',
      coverArt: art ? art.replace('/100x100','/300x300') : ''
    };
    status.textContent = 'Found it!';
    micBtn.textContent = '✅';
    document.getElementById('listenResultCard').style.display = '';
  }catch(err){
    console.error('AudD error:', err);
    if(err && err.rateLimited){
      status.textContent = "You've used today's free shared listens. Add your own free AudD token below for unlimited use.";
      document.getElementById('listenOwnTokenLinkBtn').style.display = '';
    } else if(err && err.message === 'not_signed_in'){
      status.textContent = 'Please sign in again to use Listen.';
    } else {
      status.textContent = 'Identification failed — check your connection and try again.';
    }
    micBtn.textContent = '🎤';
  }
}
document.getElementById('listenMicBtn').addEventListener('click', runListen);
document.getElementById('listenRetryBtn').addEventListener('click', ()=>{
  document.getElementById('listenResultCard').style.display = 'none';
  document.getElementById('listenMicBtn').textContent = '🎤';
  document.getElementById('listenStatus').textContent = 'Tap the mic to start listening';
});
document.getElementById('listenAddBtn').addEventListener('click', ()=>{
  const hit = window._listenHit;
  if(!hit) return;
...
async function loadPlaylist(url){
  ...
  try{
    ...
    const serviceLabel = service === 'spotify' ? 'Spotify' : service === 'apple' ? 'Apple Music' : service === 'youtube' ? 'YouTube Music' : 'Tidal';
    infoEl.innerHTML = `
      ${coverArt ? `<img src="${escapeAttr(coverArt)}" style="width:48px;height:48px;border-radius:6px;object-fit:cover;">` : ''}
      <div>
...
function renderPlaylistTrackList(listEl, tracks){
  ...
  for(let i = 0; i < max; i++){
    ...
    html += `<div class="discover-row" style="cursor:default;" data-pl-idx="${i}">
      ${t.coverArt
        ? `<img src="${escapeAttr(t.coverArt)}" style="width:36px;height:36px;border-radius:5px;object-fit:cover;flex-shrink:0;">`
