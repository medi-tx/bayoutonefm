  ...
  const collect = (resolve, reject) => {
    sb
      ...
      .then(({ data, error })=>{
        ...
        offset += PAGE;
        collect(resolve, reject);
      })
      .catch(reject);
  };
  try{
    const result = await new Promise((resolve, reject)=> collect(resolve, reject));
    return (result || []).filter(p=>p.username);
  }catch(e){
